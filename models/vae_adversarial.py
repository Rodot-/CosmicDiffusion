...
class VAEAdversarial(nn.Module):
    def __init__(
        self,
        ...
        # data_augmentation: float = 0.1,
        is_adversarial: bool = True,
        adv_epsilon: float = 5e-4,
        adv_alpha: float = 0.9,
        is_adversarial_full_model_on: bool = False,
    ) -> None:
        super().__init__()
        ...
        if self.is_adversarial:
            ...
            self.is_adversarial_full_model_on = is_adversarial_full_model_on
    ...
    # def interp_fn(self, reconstruction, alpha_interp=None, random_index=None):

    #     if alpha_interp is None:
    #         alpha_interp = (
    #             torch.rand(reconstruction.shape[0], 1).to(reconstruction.device) * 0.5
    #         )

    #         random_index = torch.randperm(reconstruction.shape[0])

    #     x_interp = (
    #         alpha_interp * reconstruction
    #         + (1 - alpha_interp) * reconstruction[random_index, :]
    #     )

    #     return x_interp, alpha_interp, random_index
    ...
    def forward(self, inputs: torch.Tensor, labels: torch.Tensor) -> VAEOutput:
        ...
        if self.training and self.is_adversarial:
            inputs.requires_grad_()
            labels_onehot = labels_onehot.type(torch.float32)
            labels_onehot.requires_grad_()
        ...
        # if self.training and self.is_adversarial:
        #     latent_observation.requires_grad_()

        if self.training and self.is_adversarial:
            latent_observation.requires_grad_()
            latent_observation.retain_grad()
    ...
    def loss_likelihood(self, observation, likelihood):
        if not self.use_elemental_mass:
            structure, abundance = (
                observation[:, :200],
                observation[:, 200:],
            )
            log_pxz_s = likelihood[0].log_prob(structure)
            log_pxz_a = likelihood[1].log_prob(
            ...
            log_pxz = likelihood[0].log_prob(observation)
            vel_loss = -torch.sum(torch.mean(log_pxz[:, :100], 0))
            reconstruction_loss = -torch.sum(torch.mean(log_pxz, 0))

        return reconstruction_loss, vel_loss

    def loss_function(
        self, vae_out: VAEOutput, is_training: bool = None
    ) -> typing.Dict[str, torch.Tensor]:
        if self.reconstruction_loss == "likelihood":
            reconstruction_loss, vel_loss = self.loss_likelihood(
                vae_out.observation, vae_out.likelihood
            )
            ...
            if self.training and self.is_adversarial:
                ...
                labels_onehot_adv = (
                    vae_out.labels_onehot
                    + self.adv_epsilon * torch.sign(vae_out.labels_onehot.grad.data)
                )
                if self.is_adversarial_full_model_on:
                    observation_adv = (
                        vae_out.observation
                        + self.adv_epsilon * torch.sign(vae_out.observation.grad.data)
                    )
                    if self.is_encoder_label_on:
                        encoder_inputs = torch.cat(
                            [
                                observation_adv.view([len(observation_adv), -1]),
                                labels_onehot_adv,
                            ],
                            axis=1,
                        )
                    else:
                        encoder_inputs = observation_adv.view(
                            [len(observation_adv), -1]
                        )
                    mu, sigma = self.encode(encoder_inputs)
                    _, _, latent_observation = self.sample(mu, sigma)
                else:  # decoder only
                    latent_observation = (
                        vae_out.latent_observation
                        + self.adv_epsilon
                        * torch.sign(vae_out.latent_observation.grad.data)
                    )
                ...
                if self.is_decoder_label_on:
                    decoder_inputs = torch.cat(
                        [latent_observation, labels_onehot_adv], axis=1
                    )
                else:
                    decoder_inputs = latent_observation

                _, *likelihood_dis_adv = self.decode(decoder_inputs)

                reconstruction_loss_adv, _ = self.loss_likelihood(
                    vae_out.observation, likelihood_dis_adv
                )

                reconstruction_loss = (
                    self.adv_alpha * reconstruction_loss
                    + (1 - self.adv_alpha) * reconstruction_loss_adv
                )
