...
class VAEMixup(nn.Module):
    """
    VAE with mixup data augmentation in the obervation space.
    """
    ...
    def __init__(
        ...
        data_augmentation_range: float = 0.2,
    ) -> None:
        super().__init__()
    ...
    def mixup(self, inputs, labels, alpha=1.0):
        # TODO: maybe we can use beta distribution instead of uniform distribution
        # TODO: is it better to move it to dataset class?
        batch_size = inputs.shape[0]
        # lam = torch.distributions.Beta(alpha, alpha).sample((batch_size, 1))
        lambda_interpol = (1 + 2 * self.data_augmentation_range) * torch.rand(
            batch_size, 1
        ) - self.data_augmentation_range
        # lam = torch.max(lam, 1 - lam)
        lambda_interpol = lambda_interpol.to(inputs.device)
        index = torch.randperm(batch_size)
        mixed_inputs = (
            lambda_interpol * inputs + (1 - lambda_interpol) * inputs[index, :]
        )
        mixed_labels = (
            lambda_interpol * labels + (1 - lambda_interpol) * labels[index, :]
        )
    ...
    def forward(self, inputs: torch.Tensor, labels: torch.Tensor) -> VAEOutput:
        ...
        if self.training:
            ...
            inputs_mixup, labels_onehot_mixup = self.mixup(inputs, labels_onehot)
            inputs = torch.cat([inputs, inputs_mixup], axis=0)
            labels_onehot = torch.cat([labels_onehot, labels_onehot_mixup], axis=0)
